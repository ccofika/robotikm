  ...
  const renderMaterialItem = ({ item }) => (
    <Card style={styles.itemCard}>
      ...
      <View style={styles.itemBody}>
        ...
        <Text style={styles.description}>Material</Text>
      </View>
    </Card>
  );
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statsRow}>
          ...
          {Object.entries(stats.byType).slice(0, 1).map(([type, data]) => (
            <View key={type} style={[styles.statCard, { borderColor: theme.colors.warning }]}>
...
const styles = StyleSheet.create({
  ...
  itemCard: { marginBottom: theme.spacing.md, borderLeftWidth: 4, borderLeftColor: theme.colors.warning },
  itemHeader: { marginBottom: theme.spacing.sm },
  quantityBadge: { backgroundColor: theme.colors.warning + '20', paddingHorizontal: theme.spacing.sm, paddingVertical: theme.spacing.xs, borderRadius: theme.borderRadius.sm, alignSelf: 'flex-start' },
  quantityText: { fontSize: theme.fontSize.sm, fontWeight: 'bold', color: theme.colors.warning },
