...
export default function WorkOrdersScreen({ navigation }) {
  ...
  const filteredWorkOrders = useMemo(() => {
    return workOrders.filter(order => {
      ...
      if (statusFilter === 'all') {
        ...
        // Filter for incomplete (not new) orders
        statusMatch = order.status === 'nezavrsen' && !isWorkOrderNew(order);
  ...
  const stats = useMemo(() => {
    ...
    const total = allTotal - completed; // Ukupno = svi osim završenih
    return {
      total,
      completed,
      pending,
        ...
        <Box className="bg-white mb-2.5 p-3 rounded-xl shadow-sm border-l-4" style={{ borderLeftColor: displayStatus.color }}>
          <VStack space="xs">
            ...
            <VStack space="xs">
              ...
              {item.details && (
                <Box className="bg-blue-50 border border-blue-200 rounded-lg p-2 mt-0.5">
                  <HStack space="xs" className="items-center">
                    <Ionicons name="information-circle" size={9} color="#2563eb" />
                    <Text style={{ fontSize: 11, color: '#2563eb', lineHeight: 14 }} numberOfLines={2}>{item.details}</Text>
                  </HStack>
                </Box>
              )}
  ...
  return (
    <Box className="flex-1 bg-gray-50">
      ...
      <HStack className="bg-white px-4 py-3 border-b border-gray-100 justify-between items-center" style={{ paddingTop: insets.top + 12 }}>
        ...
        <HStack space="xs" className="items-center">
          ...
          <Pressable
            ...
            style={{ minHeight: 44, minWidth: 44 }}
            className="bg-red-50 rounded-xl items-center justify-center active:bg-red-100"
          >
      ...
      {/* Stats Cards - Material Design 3 with Horizontal Scroll */}
      <Box className="bg-white mb-2">
        ...
        >
          ...
          <Pressable onPress={() => setStatusFilter('all')}>
            <Box
              className={`bg-blue-50 rounded-lg border ${statusFilter === 'all' ? 'border-blue-600 border-2' : 'border-blue-100'}`}
              style={{ width: 85, minHeight: 42, paddingVertical: 6, paddingHorizontal: 4, justifyContent: 'center', alignItems: 'center', marginRight: 4 }}
            >
          ...
          {/* Završeno */}
          <Pressable onPress={() => setStatusFilter('zavrsen')}>
            <Box
              className={`bg-green-50 rounded-lg border ${statusFilter === 'zavrsen' ? 'border-green-600 border-2' : 'border-green-100'}`}
              style={{ width: 85, minHeight: 42, paddingVertical: 6, paddingHorizontal: 4, justifyContent: 'center', alignItems: 'center', marginRight: 4 }}
            >
              <HStack space="xs" className="items-center" style={{ marginBottom: 2 }}>
          ...
          {/* Novi */}
          <Pressable onPress={() => setStatusFilter('new')}>
            <Box
              className={`bg-purple-50 rounded-lg border ${statusFilter === 'new' ? 'border-purple-600 border-2' : 'border-purple-100'}`}
              style={{ width: 85, minHeight: 42, paddingVertical: 6, paddingHorizontal: 4, justifyContent: 'center', alignItems: 'center', marginRight: 4 }}
            >
              <HStack space="xs" className="items-center" style={{ marginBottom: 2 }}>
          ...
          {/* Nezavršeno */}
          <Pressable onPress={() => setStatusFilter('nezavrsen')}>
            <Box
              className={`bg-yellow-50 rounded-lg border ${statusFilter === 'nezavrsen' ? 'border-yellow-600 border-2' : 'border-yellow-100'}`}
              style={{ width: 85, minHeight: 42, paddingVertical: 6, paddingHorizontal: 4, justifyContent: 'center', alignItems: 'center', marginRight: 4 }}
            >
              <HStack space="xs" className="items-center" style={{ marginBottom: 2 }}>
          ...
          {/* Odloženi */}
          <Pressable onPress={() => setStatusFilter('odlozen')}>
            <Box
              className={`bg-red-50 rounded-lg border ${statusFilter === 'odlozen' ? 'border-red-600 border-2' : 'border-red-100'}`}
              style={{ width: 85, minHeight: 42, paddingVertical: 6, paddingHorizontal: 4, justifyContent: 'center', alignItems: 'center', marginRight: 4 }}
            >
              <HStack space="xs" className="items-center" style={{ marginBottom: 2 }}>
                <Box className="w-3.5 h-3.5 rounded-full bg-red-100 items-center justify-center">
                  <Ionicons name="pause-circle" size={9} color="#dc2626" />
                </Box>
                <Text style={{ fontSize: 13, fontWeight: '700', color: '#b91c1c' }}>{stats.postponed}</Text>
              </HStack>
              <Text style={{ fontSize: 8, color: '#dc2626' }}>ODLOŽENI</Text>
            </Box>
          </Pressable>

          {/* Otkazani */}
          <Pressable onPress={() => setStatusFilter('otkazan')}>
            <Box
              className={`bg-gray-100 rounded-lg border ${statusFilter === 'otkazan' ? 'border-gray-600 border-2' : 'border-gray-300'}`}
              style={{ width: 85, minHeight: 42, paddingVertical: 6, paddingHorizontal: 4, justifyContent: 'center', alignItems: 'center', marginRight: 4 }}
            >
              <HStack space="xs" className="items-center" style={{ marginBottom: 2 }}>
                <Box className="w-3.5 h-3.5 rounded-full bg-gray-200 items-center justify-center">
                  <Ionicons name="close-circle" size={9} color="#64748b" />
                </Box>
                <Text style={{ fontSize: 13, fontWeight: '700', color: '#475569' }}>{stats.cancelled}</Text>
              </HStack>
              <Text style={{ fontSize: 8, color: '#64748b' }}>OTKAZANI</Text>
            </Box>
          </Pressable>
        </ScrollView>
      </Box>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <VStack space="md">
                ...
                <VStack space="sm">
                  ...
                  <VStack space="xs">
                    {[
                      { value: 'all', label: 'Svi nalozi', icon: 'apps' },
